      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-40 bg-dark-950/98 backdrop-blur-2xl md:hidden flex flex-col items-center justify-center gap-8"
          >
            {navLinks.map((link) => (
              <Link
                key={link.name}
                ...
                className="text-3xl font-heading font-black text-white hover:text-gold-400 transition-colors uppercase italic"
              >
                {link.name}
              </Link>
            ))}
            <Link to="contact" smooth={true} onClick={() => setIsOpen(false)}>
              <button className="bg-gold-400 text-black px-10 py-4 rounded-full font-bold text-lg uppercase tracking-widest">
                Start Today
              </button>
            </Link>
          </motion.div>
